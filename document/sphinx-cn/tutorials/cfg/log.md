...
`console`日志后端是 AimRT 官方提供的一种日志后端，使用 guard 线程打印日志, 用于将日志打印到控制台上。其所有的配置项如下：
...
`rotate_file`日志后端是 AimRT 官方提供的一种日志后端，使用 guard 线程打印日志, 用于将日志打印到文件中。其所有的配置项如下：
...
| max_file_num          | unsigned int | 可选     | 100                                | 日志文件最大数量。0 表示无上限                  |
| module_filter         | string       | 可选     | "(.\*)"                            | 模块过滤器                                      |
| pattern               | string       | 可选     | "[%c.%f][%l][%t][%n][%g:%R @%F]%v" | 日志的输出格式                                  |
