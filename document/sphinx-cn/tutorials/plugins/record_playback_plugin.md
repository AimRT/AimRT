...
| record_actions[i].options.record_enabled                         | bool         | 可选           | true    | 录制总开关：控制该 action 下所有 topic 是否可录制。实际是否录制 = 此开关为 true 且对应 topic 的 record_enabled 为 true |
...
| record_actions[i].options.topic_meta_list[j].record_enabled      | string       | 可选           | "true"  | 该 topic 的局部开关：仅当 action 的 record_enabled 为 true 且本项为 true 时才录制；未配置则按默认值启用                |
