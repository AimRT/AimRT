...
| record_actions[i].options.storage_policy.journal_mode | string        | 可选  | memory        | sqlite3 日志模式，不区分大小写，现存在 delete、truncate、persist、memory、wal、off 六种模式|
| record_actions[i].options.storage_policy.synchronous_mode | string        | 可选  |   full    | sqlite3 同步模式，不区分大小写，现存在 extra、full、normal、off 四种模式 |
