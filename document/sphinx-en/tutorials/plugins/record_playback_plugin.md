...
| record_actions[i].options.record_enabled                         | bool         | Optional                   | true    | Main recording switch: controls whether all topics under this action can be recorded. Actual recording = this switch is true AND the corresponding topic's record_enabled is true |
...
| record_actions[i].options.topic_meta_list[j].record_enabled      | string       | Optional                   | "true"  | Local switch for this topic: The topic is recorded only if both the action's record_enabled and this field are true; if not set, the default value is enabled.                    |
...
| record_actions[i].options.topic_meta_list[j].cache_last_msg      | bool         | Optional                   | "false" | Whether to cache the most recent message by default so topics that publish only one frame are preserved                                                                           |
| record_actions[i].options.extra_file_path                        | array        | Optional                   | []      | Additional file paths to bundle with each recording directory; the system copies them via the `timer_executor` executor                                                           |
...
| playback_actions[i].options.play_duration_s                      | unsigned int | Optional                   | 0       | Playback duration; only effective in imd mode. 0 means play the entire bag                                                                                                        |
