...
bool BenchmarkPublisherModule::Initialize(aimrt::CoreRef core) {
  ...
  try {
    ...
    bool ret = aimrt::channel::RegisterPublishType<example_ros2::msg::BenchmarkSignal>(signal_publisher_);
    ...
    for (uint32_t ii = 0; ii < max_topic_number_; ++ii) {
      ...
      bool ret = aimrt::channel::RegisterPublishType<example_ros2::msg::BenchmarkMessage>(publisher);
      AIMRT_CHECK_ERROR_THROW(ret, "Register publish type failed.");
...
void BenchmarkPublisherModule::MainLoop() {
  try {
    ...
    for (size_t ii = 0; ii < 10; ++ii) {
      example_ros2::msg::BenchmarkSignal begin_signal;
      begin_signal.set__status(example_ros2::msg::BenchmarkStatus::WARM_UP);
...
void BenchmarkPublisherModule::StartMultiTopicPlan(uint32_t plan_id, BenchPlan plan) {
  ...
  example_ros2::msg::BenchmarkSignal begin_signal;
  begin_signal.set__status(example_ros2::msg::BenchmarkStatus::BEGIN);
  begin_signal.set__mode("multi-topic");
  begin_signal.set__bench_plan_id(plan_id);
  begin_signal.set__topic_number(plan.topic_number);
  begin_signal.set__parallel_number(1);
  begin_signal.set__send_num(plan.msg_count);
  begin_signal.set__message_size(plan.msg_size);
  begin_signal.set__send_frequency(plan.channel_frq);

  AIMRT_INFO("Publish benchmark start signal, data: {}", example_ros2::msg::to_yaml(begin_signal));
  aimrt::channel::Publish(signal_publisher_, begin_signal);
    ...
    publish_executor.Execute(
        [this, publisher, plan, task_promise{std::move(task_promise)}]() mutable {
          ...
          for (; send_count < plan.msg_count; ++send_count) {
            ...
            msg.set__seq(send_count);
            msg.set__timestamp(aimrt::common::util::GetCurTimestampNs());
  ...
  // publish end signal
  example_ros2::msg::BenchmarkSignal end_signal;
  end_signal.set__status(example_ros2::msg::BenchmarkStatus::END);
  end_signal.set__bench_plan_id(plan_id);

  AIMRT_INFO("Publish benchmark end signal, data: {}", example_ros2::msg::to_yaml(end_signal));
...
void BenchmarkPublisherModule::StartParallelPlan(uint32_t plan_id, BenchPlan plan) {
  ...
  begin_signal.set__status(example_ros2::msg::BenchmarkStatus::BEGIN);
  begin_signal.set__bench_plan_id(plan_id);
  begin_signal.set__mode("parallel");
  begin_signal.set__topic_number(1);
  begin_signal.set__parallel_number(plan.parallel_number);
  begin_signal.set__send_num(plan.msg_count);
  begin_signal.set__message_size(plan.msg_size);
  begin_signal.set__send_frequency(plan.channel_frq);
    ...
    publish_executor.Execute(
        [this, publisher, plan, last_send_counts, ii, task_promise{std::move(task_promise)}]() mutable {
          ...
          for (; send_count < messages_to_send; ++send_count) {
            ...
            msg.set__seq(last_send_counts[ii] + send_count);
            msg.set__timestamp(aimrt::common::util::GetCurTimestampNs());
